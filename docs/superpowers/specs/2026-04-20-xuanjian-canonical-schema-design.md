...
1. 这一版模板刻意压低了单次专注的基础收益，避免 `主修功法倍率 + cultivationAttainment 修正` 叠乘后把前中期节奏推得过快。
2. `60` 分钟是主修为结算的起算档，后续依靠更长专注时长拉开收益，而不是单纯堆高短时任务的数值。
3. 由于这组基础值明显低于前一版，真正进入实现前应至少做一次实机回放，确认上一节的境界需求表不会把整体节奏压得过慢。
...
当前旧系统是“通用九境”，并不与玄鉴六境一一等价。因此 `V1` 不再尝试做“旧修为 -> 新修为”的数值折算，而是明确采用“修仙进度清零重开、存储壳字段保留”的方案。

执行规则：

1. 迁移脚本只负责为旧用户补写 canonical 初始状态，不再按旧 `spiritualPower` 推导新境界。
2. canonical 初始状态统一为：
   - `realmId = realm.taixi`
   - `currentPower = 0`
   - `cultivationAttainment = 0`
   - `mainMethodId = method.starter_tuna`
3. 与专注系统直接相关、又不破坏平衡的信息允许保留，例如 `focusStreak`。
4. 新系统上线后，一切修仙状态判断都以 canonical `RealmDefinition` 为准；旧字段只作为兼容壳和展示缓存。

这样做的理由是：

1. 旧九境与玄鉴六境并不共构，强行折算只会制造伪精确。
2. canonical 的主循环已经切换为“专注 -> 修为 -> 道行 -> 奇遇 -> 破境”，旧修为直接带入会污染新平衡。
3. 与其给高阶旧号做一套无法自证正确的压缩表，不如明确告知“玄鉴修仙线重新开局”，产品语义更干净。
