...
- **修成标志/核心仪式**：化去六轮，凝结仙基
  ...
  3. 流光在气海中卷起气浪，大量消耗真元
  4. 流光吸饱真元后碰撞凝结成仙基
- **结果决定因素**：主修功法决定仙基类型、主道统与底蕴潜力；丹药、灵物与破境术只影响冲关过程、代价与成败把握，不改写仙基归属
...
| 筑基→紫府 | **高风险收集突破** | 必须集齐：玄阶丹药×2 + 玄阶灵材×2 + 玄阶功法×1 + 玄阶破境法门×1（化神通秘法）；当前 runtime 按四关流程求解，首神通由仙基自动映射，失败损伤修为与道行 |
...
- `branchCultivationAttainments` — 分支修炼成就，第一版仍为空对象，不参与运行时倍率
- `cooldowns` — 冷却计时器，空对象
...
- `cultivationAttainment` 是当前唯一道行数值，不再额外拆出第二套“分支道行”账本
- `mainDaoTrack` 运行时统一使用 `universal` 表示“通用 / 未定道基”；旧存档里的 `neutral` 会在读取时自动收敛为 `universal`
- 胎息、练气阶段 `/realm` 显示 `☯️ 当前道统：通用`
- `练气 -> 筑基` 成功突破时，由当前主修功法的筑基结果映射锁定 `mainDaoTrack`、`foundationId` 与对应筑基主修；`selectedBreakthroughMethodId` 只决定冲关过程，不改结果
- 筑基后主道统只强化匹配 `lineageTag` 的主修功法、法门、神通
- `branchCultivationAttainments` 第一版仅保留为 schema 预留字段，不参与运行时判断

### 紫府四关与后续神通口径

- `selectedBreakthroughMethodId` 表示本次破境法门，只影响成功率、稳定度、消耗、副作用与副产物，不决定神通类型
- `realm_zhuji_to_zifu` 使用 `foundationId -> firstDivinePowerId` 自动授予首个神通，并记录四关过程
- `zifu_divine_power` 用于紫府内后续神通冲关；当前只实现第二道神通的最小闭环
- `branchChoice / branchProofs` 只在 `zifu_divine_power` 生效，用来校验目标神通、主修功法覆盖范围和分支证明
- 紫府冲关失败只扣减 `currentPower` 与 `cultivationAttainment`，不做立死、删号、废基或长期 debuff
- 第三到第五道神通复杂互斥、以及 `紫府 -> 金丹` 的求金/求闰，仍留到金丹冲刺阶段处理
...
Phase C 之后的奇遇战已不再只停留在 starter 单模板，而是开始进入统一奇遇框架：前中期共享宝物池，守宝敌人通过原型与境界参数实例化，玩家可在守宝奇遇中选择 `离开` 或 `争抢`，同时 `V1` 的修为主循环仍保持不变。

### 新增运行时文件

- `src/types/cultivationCombat.ts` — 主动战斗的最小类型契约
- `src/config/xuanjianCombat.ts` — 统一奇遇配置、敌人原型、混池宝物与文案 formatter
...
- 奇遇池按宝物混池，不再按 `胎息 / 练气 / 筑基` 维护三套独立掉落表
- 高价值宝物通常会生成守宝敌人，守宝敌人由 `原型 + 境界 + 小阶 + seed` 实例化
- 玩家在守宝奇遇中始终拥有 `离开 / 争抢` 两个选择
- `离开` 时宝物直接消失，无伤无损，不进入战斗
- `争抢` 时才进入现有 `CombatService -> CombatResolver -> CombatRewardBridge` 战斗链路
- 混池掉落的高阶法门 / 神通会先以 `manual.*` 形式入包为待参悟条目，不会直接变成立刻可配装内容
...
- `/dev_encounter_set offer <count>` — 仅测试环境可用，强制后续专注进入守宝奇遇选择流
